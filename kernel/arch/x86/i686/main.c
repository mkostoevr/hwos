...
void arch_kmain(struct multiboot* mboot, u32 mboot_magic) {
    ...
    free_map_entry_t phys_free_map[memmap_entries_count];
    ...
    for (int i = 0; i < memmap_entries_count; i++) {
        if (memmap[i].type == MULTIBOOT_MEMORY_AVAILABLE) {
            if (memmap[i].base_addr + memmap[i].length >= DONT_USE_UP_TO) {
                phys_free_map[phys_free_map_len++] = (free_map_entry_t){(uintptr_t)memmap[i].base_addr, (size_t)memmap[i].length};
            }
        }
