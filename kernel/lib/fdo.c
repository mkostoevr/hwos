...
void fdo_set_text_color(int text_color) {
    #if defined ARCH_i686
    vgatext_setcolor(vgatext_entry_color(text_color, BLACK));
    #else
    #error Error
    #endif
}
...
void fdo_set_default_text_color() {
    #if defined ARCH_i686
    vgatext_setcolor(vgatext_entry_color(LIGHT_GREY, BLACK));
    #else
    #error Error
    #endif
}
    ...
    while (format[i]) {
        if (format[i] == '%') {
            ...
            case 'x':
                ...
                break;
            case 'w':
                fdo_set_text_color(va_arg(args, int));
                break;
            case 'y':
