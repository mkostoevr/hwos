...
def collect_sources(folder = "kernel", recursive = True):
    return (collect_files(".cpp", folder = folder, recursive = recursive) +
            collect_files(".c", folder = folder, recursive = recursive) +
...
def clang(source, flags):
    ...
    os.system(f"clang {flags} -I . -c {source} -o {source_object}")
    return source

def compile_c(source):
    global cflags
    return clang(source, cflags)

def compile_cpp(source):
    global cppflags
    return clang(source, cppflags)

def compile_S(source):
    global aflags
    return clang(source, aflags)
...
if __name__ == '__main__':
    ...
    cflags = f"-ffreestanding -mno-mmx -mno-sse -mno-sse2 -mno-3dnow -fno-rtti -fno-exceptions -fno-threadsafe-statics -fno-strict-aliasing -fno-stack-protector -Wall -Wextra -g -DARCH_{ARCH} --target={ARCH}-unknown-linux-gnu"
    ...
    for source in cpp_sources:
        pool.apply_asyncs(compile_cpp, args = (source,), callback = compile_success, error_callback = compile_failure)

    for source in c_sources:
        pool.apply_async(compile_c, args = (source,), callback = compile_success, error_callback = compile_failure)
